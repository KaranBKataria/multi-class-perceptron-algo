...
for $\mathbf{x}, \mathbf{x}' \in \mathcal{X} \subseteq \mathbb{R}^{n}$, $\ell \in \mathbb{R}$ which is the lengthscale parameter and $\lVert \dot \rVert_{s}$ is the $\ell_{2}$/Euclidean norm.
